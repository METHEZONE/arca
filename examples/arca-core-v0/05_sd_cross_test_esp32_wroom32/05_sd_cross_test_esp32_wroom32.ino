#include <Wire.h>
#include <SPI.h>
#include <SD.h>
#include <Adafruit_GFX.h>
#include <Adafruit_SSD1306.h>

#define SCREEN_WIDTH 128
#define SCREEN_HEIGHT 64
#define OLED_RESET -1

// Classic ESP32 DevKit / LOLIN D32 defaults.
#define OLED_SDA 21
#define OLED_SCL 22

#define SD_CS 5
#define SD_SCK 18
#define SD_MOSI 23
#define SD_MISO 19

Adafruit_SSD1306 display(SCREEN_WIDTH, SCREEN_HEIGHT, &Wire, OLED_RESET);

void show(const char *a, const char *b = "", const char *c = "", const char *d = "") {
  display.clearDisplay();
  display.setTextColor(SSD1306_WHITE);
  display.setTextSize(1);
  display.setCursor(0, 0);
  display.println(a);
  display.println(b);
  display.println(c);
  display.println(d);
  display.display();
}

bool trySd(uint32_t hz) {
  char speed[32];
  snprintf(speed, sizeof(speed), "%lu Hz", (unsigned long)hz);
  show("ESP32 SD cross", "VCC 3V3", speed);
  Serial.print("Trying SD at ");
  Serial.println(speed);

  SD.end();
  SPI.end();
  delay(100);
  pinMode(SD_CS, OUTPUT);
  digitalWrite(SD_CS, HIGH);
  SPI.begin(SD_SCK, SD_MISO, SD_MOSI, SD_CS);
  delay(100);

  if (!SD.begin(SD_CS, SPI, hz)) {
    Serial.println("SD.begin failed");
    return false;
  }

  uint64_t mb = SD.cardSize() / (1024ULL * 1024ULL);
  File f = SD.open("/arca_esp32_ok.txt", FILE_WRITE);
  if (!f) {
    show("SD mounted", "file open failed");
    Serial.println("file open failed");
    return false;
  }

  f.println("ARCA ESP32-WROOM32 SD OK");
  f.print("speed=");
  f.println(hz);
  f.close();

  char sizeLine[32];
  snprintf(sizeLine, sizeof(sizeLine), "size %lu MB", (unsigned long)mb);
  show("SD OK", speed, sizeLine, "wrote esp32 ok");
  Serial.println("SD OK and wrote /arca_esp32_ok.txt");
  return true;
}

void setup() {
  Serial.begin(115200);
  delay(1000);

  Wire.begin(OLED_SDA, OLED_SCL);
  display.begin(SSD1306_SWITCHCAPVCC, 0x3C);
  show("ESP32 SD cross", "OLED 21/22", "SD 5/18/23/19");

  uint32_t speeds[] = {100000, 400000, 1000000, 4000000};
  for (uint8_t i = 0; i < sizeof(speeds) / sizeof(speeds[0]); i++) {
    if (trySd(speeds[i])) {
      return;
    }
    delay(300);
  }

  show("SD failed on ESP32", "module/card issue", "not S3-only");
  Serial.println("SD failed on classic ESP32 too.");
}

void loop() {
}
