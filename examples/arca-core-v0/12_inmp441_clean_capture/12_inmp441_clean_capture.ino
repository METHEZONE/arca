...
#define WAV_BYTES (44 + SAMPLE_COUNT * 2)
...
int16_t inmp441ToS16(int32_t raw) {
  // INMP441 emits signed 24-bit samples left-justified in a 32-bit I2S slot.
  int32_t s24 = raw >> 8;
  int32_t s16 = s24 >> 8;
...
void printLevel(int16_t *buf, size_t n) {
  int16_t mn = 32767;
  int16_t mx = -32768;
  uint64_t sumSq = 0;
  ...
  float rms = sqrtf((float)sumSq / max((size_t)1, n));
  Serial.printf("level min=%d max=%d rms=%.1f\n", mn, mx, rms);
}
...
void recordSamples() {
  ...
  while (written < SAMPLE_COUNT) {
    ...
    for (size_t i = 0; i < count && written < SAMPLE_COUNT; i++) {
      samples[written++] = inmp441ToS16(scratch[i]);
    }
  }
  printLevel(samples, SAMPLE_COUNT);
}
...
void streamWav() {
  ...
  Serial.write((const uint8_t *)samples, SAMPLE_COUNT * sizeof(int16_t));
  Serial.flush();
  delay(50);
  Serial.print("\nARCA_WAV_END\n");
  Serial.flush();
...
void setup() {
  ...
  if (!samples) {
    Serial.println("ARCA_INMP441_CLEAN_CAPTURE ps_malloc failed");
    while (true) delay(1000);
  }
  setupI2S();
  Serial.println("ARCA_INMP441_CLEAN_CAPTURE_READY send r");
  Serial.println("pins BCLK=16 WS=17 DIN=15 LR=3V3 10s 16kHz s16le wav");
}
