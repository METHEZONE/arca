...
@Suite struct ClaudeSummarizerTests {
    ...
    @Test func requestBodyForcesTheStructuredTool() throws {
        ...
        #expect(required.isSuperset(of: ["title", "tldr", "sections", "decisions",
    ...
    @Test func promptDemandsSpecificsRatherThanConciseness() {
        ...
        // The field guide has to name every field the schema requires, or the
        // model fills the easy ones and leaves the rest empty.
        #expect(system.contains("tldr"))
        #expect(system.contains("sections"))
        #expect(system.contains("decisions"))
        #expect(system.contains("actionItems"))
        #expect(system.contains("openQuestions"))
        // Detail is the point of the task: numbers and names must survive.
        #expect(system.contains("numbers"))
        ...
        // The anchor date is what turns "다음 주 금요일까지" into a real due date.
        #expect(user.contains("Today's date:"))
    }
