...
struct BriefingCard: View {
        ...
        VStack(alignment: .leading, spacing: 10) {
            HStack(spacing: 8) {
                ...
                Button {
                    ...
                    Label(ops.briefing == nil
                          ? L("Brief me", ko: "브리핑 받기")
                          : L("Refresh", ko: "새로고침"),
            ...
            if ops.isBriefing {
                HStack(spacing: 8) {
                    ...
                    Text(L("Reading your day…", ko: "하루를 읽는 중…"))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                ...
                Text(L("Calendar, open quests, today's sessions — one tap and I'll lay out your day.",
                       ko: "캘린더, 열린 퀘스트, 오늘의 세션 — 한 번만 누르면 하루를 정리해 드려요."))
                    .font(.caption)
                    .foregroundStyle(.secondary)
            }
    ...
    private func briefingBody(_ briefing: AmbientOps.Briefing) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            section(L("Do", ko: "할 일"), items: briefing.today, symbol: "flag.fill",
                    tint: ArcaSkins.current.mid)
            if !briefing.asks.isEmpty {
                section(L("Ask", ko: "요청할 것"), items: briefing.asks, symbol: "person.2.fill",
                        tint: .blue)
            }
            if !briefing.done.isEmpty {
                section(L("Done", ko: "끝낸 일"), items: briefing.done, symbol: "checkmark.seal.fill",
                        tint: .green)
            }
        }
    }
