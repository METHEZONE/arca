...
const calendar = async (p:PromisePacket,key:string) => ({adapter:"demo-calendar" as const,externalId:`https://github.com/demo/arca/pull/${parseInt(createHash("sha256").update(key).digest("hex").slice(0,4),16)%900+100}`,observedAt:new Date().toISOString(),digest:createHash("sha256").update(`${p.id}:${p.slot.start}:${p.slot.end}`).digest("base64url")});
